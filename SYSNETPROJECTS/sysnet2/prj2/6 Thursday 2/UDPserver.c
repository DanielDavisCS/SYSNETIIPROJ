...
void run_Server(int sockfd){
  ...
  int byteReceivedCount = 1, shutdown = 0;
 ...
 */
int handleMessage(int sockfd, struct sockaddr_in cliaddr, char *recvMesg){
  int byteSentCount = 0, shutdown = 0; 
  ...
  if(shutdown == 0) {
	...
	byteSentCount = sendto(sockfd, sendMesg, MAX_MESSAGE, 0, (struct sockaddr *) &cliaddr, clilen);
	...
	printf("The Sever Is Being Powered OFF !!");
	printf("\n***************************************************\n\n");
	return -1;
  }
}
