 ...
 **************************************************
 */
struct sockaddr_in listen_On_Socket(int sockfd, struct sockaddr_in servaddr){
  int servaddrlen = sizeof(struct sockaddr_in);
  if(getsockname(sockfd, (struct sockaddr *) &servaddr, (socklen_t *) &servaddrlen) == -1)
	printErrorMessage("Cannot Listen To This Port");
  if(listen(sockfd, MAX_NUM_LISTENER_ALLOWED) == -1)
...
void receiveMessage(int sockfd){
  ...
  //used for storing client information
  ...
  while(!shutdown) 
  {
	...
	socklen_t clilen = sizeof(clientStruct_p->clientaddr);
  	...
  	byteReceivedCount = recvfrom(sockfd, recvMesg, MAX_MESSAGE, 0,(struct sockaddr *) &cliaddr, &clilen);

 	//only if client sent a message does it need processing
	if(byteReceivedCount > 0)
 		shutdown = handleMessage(clientStruct_p, recvMesg);
   }
  close(sockfd);
}
...
int modifyMessage(char *recvMesg, char *send){
  ...
  else if(!strcmp(recvMesg, "<loadavg/>"))
   	loadavgMessage(recvMesg, sendMesg); 
  //handle <shutdown/> messages
  else if(!strcmp(recvMesg, "<shutdown/>"))
        return -1;
